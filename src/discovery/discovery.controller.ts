...
function toPodcastResponse(
  ...
  const category = cat && typeof cat === 'object' && 'slug' in cat
    ? { id: (cat as PopulatedCategory)._id?.toString(), slug: (cat as PopulatedCategory).slug, name: (cat as PopulatedCategory).name }
    : undefined;
  const sub = doc.subcategoryId as unknown;
  const subcategory = sub && typeof sub === 'object' && 'slug' in sub
    ? { id: (sub as PopulatedCategory)._id?.toString(), slug: (sub as PopulatedCategory).slug, name: (sub as PopulatedCategory).name }
    : undefined;
...
function toEpisodeResponse(doc: EpisodeDocument, podcastCoverUrl?: string | null) {
  ...
  const podcastIdStr =
    typeof pid === 'object' && pid && '_id' in pid
      ...
      : String(pid);
  const podcast = typeof pid === 'object' && pid && 'coverUrl' in pid ? (pid as { coverUrl?: string | null }) : null;
