...
export class FollowsController {
    ...
    async follow(@Req() req: Request & { user: UserDocument }, @Param('id') id: string) {
    ...
    async unfollow(@Req() req: Request & { user: UserDocument }, @Param('id') id: string) {
        await this.followsService.unfollow(req.user._id, id);
    }

    @Public()
    @Get(':id/stats')
    async getStats(@Param('id') id: string, @Req() req: Request & { user?: UserDocument }) {
